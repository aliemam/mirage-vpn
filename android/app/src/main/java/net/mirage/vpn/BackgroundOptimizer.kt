...
class BackgroundOptimizer(
    ...
    private val onSwitchConfig: suspend (ProxyConfig) -> Unit
) {
    companion object {
    ...
    private suspend fun runOptimizationCycle() = withContext(Dispatchers.IO) {
        ...
        val allConfigs = configRepository?.getAllConfigs() ?: emptyList()
        ...
        var bestConfig: ProxyConfig? = null
     ...
     */
    private fun getNextCandidates(allConfigs: List<ProxyConfig>, currentConfig: ProxyConfig): List<ProxyConfig> {
     ...
     */
    private suspend fun measureConfigDelay(config: ProxyConfig): Long = withContext(Dispatchers.IO) {
        try {
